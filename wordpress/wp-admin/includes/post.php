...
function edit_post($post_data = null) {
    ...
    $arr_ifexist = get_post_meta($post_ID);
    if ($arr_ifexist["_elasticsearch_id"][0]) {
        try {
            $es_id = $arr_ifexist["_elasticsearch_id"][0];
            $project = new Project();
            $project->set_ElasticSearch_Server("api.dev.epals.com:9200");
            $project->load($es_id);
            $metadata_array = Array();
            $project->setProjectname($post_data["post_title"]);
            $project->setDescription($post_data["content"]);
            $project->setMetadata($metadata_array);
            $project->update();
        } catch (Exception $e) {
            wp_die(__("can't connect to ElasticSearch Server."));
        }
    } else {
    ...
    if (!$arr_ifexist["_elasticsearch_id"][0]) {
        add_post_meta($post_ID, '_elasticsearch_id', $project_id);
    }
...
function wp_write_post() {
    ...
    add_post_meta($post_ID, '_elasticsearch_id', 'chytest');
